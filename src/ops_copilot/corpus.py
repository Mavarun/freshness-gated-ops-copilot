...
def load_documents(path: str | Path | None = None) -> list[Document]:
    ...
    # Optional canary farm docs (prompt-injection probes), loaded when present.
    canary_src = CANARY_CORPUS if path is None else None
    if canary_src is not None and canary_src.is_file() and Path(src).resolve() == DEFAULT_CORPUS.resolve():
        with canary_src.open(encoding="utf-8") as handle:
            for line_no, raw in enumerate(handle, start=1):
                ...
                try:
                    ...
                    raise ValueError(f"{canary_src}:{line_no}: invalid JSON") from exc
                missing = {"doc_id", "title", "body", "updated_at", "source_system"} - set(row)
                if missing:
                    raise ValueError(f"{canary_src}:{line_no}: missing fields {sorted(missing)}")
                docs.append(
                    Document(
                        doc_id=str(row["doc_id"]),
                        title=str(row["title"]),
